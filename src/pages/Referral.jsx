    ...
    <div className="p-4 md:p-8 max-w-3xl mx-auto">
        ...
        <div className="flex gap-4">
          ...
          <div>
            <div className="flex gap-2 items-center mb-1">
              <h2 className="text-lg font-bold text-white">Earn {REFERRAL_REWARD.label}</h2>
            ...
            <p className="text-sm text-white/55 leading-relaxed">During beta everyone is free. After paid launch, 3 verified paying subscribers unlock Lifetime TitanOS Premium.</p>
          </div>
        </div>
      </motion.section>
      ...
      <section className="glass rounded-2xl p-5 mb-5">
        <div className="flex justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-white">{stats.progressLabel}</p>
            <p className="text-xs text-white/40 mt-1">{stats.rewardUnlocked ? "Lifetime Premium unlocked!" : `${stats.required - stats.completedPaying} more verified subscriber${stats.required - stats.completedPaying === 1 ? "" : "s"} to go`}</p>
          </div>
          <span className="text-titan-cyan font-bold">{stats.completedPaying}/{stats.required}</span>
        </div>
        <div className="h-2 bg-white/10 rounded-full overflow-hidden mt-4"><div className="h-full bg-titan-cyan transition-all duration-500" style={{ width: `${progress}%` }} /></div>
      </section>

      <section className="glass rounded-2xl p-5 mb-5">
        <p className="text-xs uppercase tracking-wider text-white/40 mb-2">Your unique referral code</p>
        <p className="font-mono font-bold tracking-widest text-titan-cyan">{code}</p>
        <div className="flex gap-2 mt-4">
          <Input readOnly value={referralLink} className="bg-[#1A1A1C] border-white/5 text-white/55 rounded-xl h-10 text-xs" />
          <Button onClick={handleCopy} variant="outline" className="border-white/10 text-white rounded-xl h-10 shrink-0 gap-2">
      ...
      <section className="glass rounded-2xl p-5 mb-5">
        <h3 className="font-semibold text-white flex gap-2 items-center mb-3"><Mail className="w-4 h-4 text-titan-cyan" /> Invite by email</h3>
        <form onSubmit={handleSendInvite} className="flex gap-2">
          <Input value={email} onChange={(event) => setEmail(event.target.value)} placeholder="friend@example.com" type="email" className="bg-[#1A1A1C] border-white/5 text-white rounded-xl h-10" />
        ...
        <p className="text-xs text-white/35 flex gap-1.5 items-center mt-3"><ShieldCheck className="w-3.5 h-3.5" /> Self-referrals blocked to protect the program.</p>
      </section>
      ...
      <section className="glass rounded-2xl p-5 mt-5">
        <h3 className="font-semibold text-white flex items-center gap-2 mb-3"><Users className="w-4 h-4 text-titan-cyan" /> Referral history</h3>
        {referrals.length ? <div className="space-y-2">{referrals.map((row) => <div key={row.id} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0"><div><p className="text-sm text-white/80">{row.referred_email || "Referral"}</p><p className="text-xs text-white/35">{new Date(row.created_at || row.created_date || Date.now()).toLocaleDateString()}</p></div><span className={`text-xs ${row.is_paying || row.status === "completed" ? "text-titan-cyan" : "text-titan-amber"}`}>{row.is_paying || row.status === "completed" ? "Verified paying" : row.status === "signed_up" ? "Signed up" : "Pending"}</span></div>)}</div> : <p className="text-sm text-white/40">Your sent invitations will appear here.</p>}
      </section>
    </div>
  );
}
...
function ReferralList({ title, icon: Icon, rows, empty, successful = false }) {
  return <section className="glass rounded-2xl p-5"><h3 className="font-semibold text-white flex gap-2 items-center mb-3"><Icon className={`w-4 h-4 ${successful ? "text-titan-cyan" : "text-titan-amber"}`} /> {title}</h3>{rows.length ? <div className="space-y-2">{rows.map((row) => <p key={row.id} className="text-sm text-white/65 truncate py-1 border-b border-white/5 last:border-0">{row.referred_email || "Referral"}</p>)}</div> : <p className="text-sm text-white/40">{empty}</p>}</section>;
}
