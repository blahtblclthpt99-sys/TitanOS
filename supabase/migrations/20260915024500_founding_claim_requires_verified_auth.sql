...
BEGIN
  ...
  -- slot. This makes the profile INSERT trigger harmless for unconfirmed email
  -- signups while preserving already-confirmed OAuth/auto-confirmed accounts.
  IF NOT EXISTS (
    SELECT 1
    FROM auth.users u
...
-- Profile creation happens before an email-confirmation interaction. Once Auth
-- records that verification, claim the Founding slot exactly once. The claim
-- function itself remains the authority and serializes concurrent claims.
CREATE OR REPLACE FUNCTION public.auth_user_claim_founding_after_verification()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.auth_user_claim_founding_after_verification() FROM PUBLIC, anon, authenticated;

DROP TRIGGER IF EXISTS trg_auth_user_claim_founding_after_verification ON auth.users;
CREATE TRIGGER trg_auth_user_claim_founding_after_verification
  ...
  EXECUTE FUNCTION public.auth_user_claim_founding_after_verification();
