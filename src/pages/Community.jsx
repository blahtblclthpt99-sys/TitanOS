...
export default function Community() {
  ...
  return <div className="relative p-4 md:p-8 max-w-6xl mx-auto pb-32">
    ...
    <div className="relative"><PageHeader title="Community" subtitle="Share professional wins with your local service network" />
      ...
      <div className="titan-surface p-4 border border-titan-cyan/15 flex gap-3 mb-6"><ShieldCheck className="w-5 h-5 text-titan-cyan shrink-0 mt-0.5" /><div className="text-sm text-muted-foreground space-y-1"><p>Privacy first: posts never show customer names, companies, addresses, or dollar amounts.</p><p>Keep it professional — foul language is blocked. Limit: {COMMUNITY_POST_LIMIT} posts per 12 hours.</p></div></div>
      <div className="grid lg:grid-cols-[minmax(0,1fr)_300px] gap-6">
        <div className="space-y-5">
          {canCompose ? <form onSubmit={submitPost} className="titan-surface p-5 border border-border"><div className="flex items-center justify-between gap-2 mb-3"><div className="flex items-center gap-2"><Sparkles className="w-4 h-4 text-titan-cyan" /><p className="font-semibold text-foreground">Share an update</p></div><p className="text-xs text-muted-foreground">{quota.remaining}/{COMMUNITY_POST_LIMIT} left (12h)</p></div><Textarea required maxLength={500} rows={3} value={compose.description} onChange={(e) => setCompose((current) => ({ ...current, description: e.target.value }))} placeholder="Share a project win or useful tip. Keep customer details private." className={fieldClass} /><div className="grid sm:grid-cols-2 gap-3 mt-3"><select value={compose.category} onChange={(e) => setCompose((current) => ({ ...current, category: e.target.value }))} className={fieldClass}>{SERVICE_CATEGORIES.map((value) => <option key={value}>{value}</option>)}</select><div className="relative"><ImagePlus className="absolute left-3 top-3 w-4 h-4 text-muted-foreground" /><Input type="url" value={compose.photoUrl} onChange={(e) => setCompose((current) => ({ ...current, photoUrl: e.target.value }))} placeholder="Photo URL (optional)" className={`${fieldClass} pl-9`} /></div></div><Button disabled={saving || quota.remaining <= 0} type="submit" className="mt-3">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : quota.remaining <= 0 ? "Limit reached (12h)" : <><Send className="w-4 h-4 mr-2" />Share Post</>}</Button></form> : <div className="titan-surface p-5 border border-border"><p className="font-semibold text-foreground">Join the conversation</p><p className="text-sm text-foreground/45 mt-1">Enable Community sharing in your privacy settings to share updates.</p><Link to="/settings" className="inline-block mt-3 text-sm font-medium text-titan-cyan hover:text-foreground">Open Privacy Settings</Link></div>}
          {loading ? <PageLoader variant="list" label="Loading posts" /> : posts.length ? posts.map((post) => <article id={`post-${post.id}`} key={post.id} className="titan-surface overflow-hidden border border-border"><div className="p-5"><div className="flex justify-between gap-3"><div><p className="font-semibold text-foreground">@{post.author_username || "localpro"}</p><p className="text-xs text-muted-foreground mt-1">{locationLabel(post.city, post.state) || "Local community"} · {post.category}</p></div><span className="text-xs text-muted-foreground whitespace-nowrap">{timeAgo(post.created_date || post.created_at)}</span></div><p className="text-sm text-foreground/85 leading-relaxed whitespace-pre-line mt-4">{post.description}</p></div>{post.photos?.length ? <div className="grid grid-cols-2 gap-px bg-muted">{post.photos.map((photo, index) => <img key={`${photo}-${index}`} src={photo} alt="" className="w-full h-48 object-cover" />)}</div> : null}<div className="p-3 flex gap-2 border-t border-border flex-wrap"><button onClick={() => like(post)} className={`flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg transition-colors ${liked.has(post.id) ? "bg-red-400/10 text-red-400" : "text-foreground/45 hover:bg-muted hover:text-foreground"}`}><Heart className={`w-4 h-4 ${liked.has(post.id) ? "fill-current" : ""}`} />{post.like_count || 0}</button><button onClick={() => toggleComments(post.id)} className="flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg text-foreground/45 hover:bg-muted hover:text-foreground"><MessageCircle className="w-4 h-4" />{post.comment_count || 0} Comments</button><button onClick={() => sharePost(post)} className="flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg text-foreground/45 hover:bg-muted hover:text-foreground"><Share2 className="w-4 h-4" />Share</button>{(String(post.author_id) === String(user?.id) || String(post.created_by_id) === String(user?.id)) && <DeleteButton label="this post" onDelete={async () => { await deleteCommunityPost(user.id, post.id); setPosts((current) => current.filter((item) => item.id !== post.id)); }} />}</div>{openComments.has(post.id) && <div className="p-4 border-t border-border bg-black/10 space-y-3">{comments[post.id]?.map((comment) => <p key={comment.id} className="text-sm text-muted-foreground"><span className="font-medium text-foreground">@{comment.author_username}</span> {comment.body}</p>)}<form onSubmit={(event) => submitComment(event, post)} className="flex gap-2"><Input value={drafts[post.id] || ""} onChange={(event) => setDrafts((current) => ({ ...current, [post.id]: event.target.value }))} placeholder="Add a comment…" className={fieldClass} /><Button disabled={saving} type="submit" size="icon" className="shrink-0"><Send className="w-4 h-4" /></Button></form></div>}</article>) : (
            <EmptyState
              className="py-10"
              icon={Sparkles}
              title="The feed is just getting started"
              description="No posts yet. Share a privacy-safe update to start the conversation — never include customer names or dollar amounts."
              actionLabel={canCompose ? "Share an update" : "Open privacy settings"}
              onAction={
                canCompose
                  ? () => document.querySelector("textarea")?.focus()
                  : undefined
              }
              actionTo={canCompose ? undefined : "/settings"}
            />
          )}
        </div>
        <aside className="titan-surface p-5 border border-border h-fit lg:sticky lg:top-20"><div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" /><h2 className="font-semibold text-foreground">Live Activity</h2></div><p className="text-xs text-muted-foreground mt-1">Updates refresh every 20 seconds.</p><div className="mt-4 space-y-4">{activityItems.length ? activityItems.map((item) => <div key={item.id} className="border-l border-titan-cyan/30 pl-3"><p className="text-sm text-muted-foreground">{activityLine(item)}</p>{item.created_at && <p className="text-xs text-muted-foreground mt-1">{timeAgo(item.created_at || item.created_date)}</p>}</div>) : <EmptyState className="py-6" title="No live activity yet" description="When pros share updates, they’ll show up here." />}</div></aside>
      </div>
    </div>
  </div>;
}
