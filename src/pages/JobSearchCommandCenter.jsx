...
export default function JobSearchCommandCenter() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Couldn't load job search", description: error.message || "Please try again." });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const track = async (job, state) => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Couldn't update opportunity", description: error.message || "Please try again." });
    } finally {
      setBusyId(null);
    }
  };
    ...
    <PageShell maxWidth="xl" className="space-y-6">
      ...
      <section className="titan-surface p-5 space-y-4" aria-labelledby="job-search-filters">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex gap-2"><Button variant="outline" onClick={rememberSearch}><BookmarkPlus className="mr-2 h-4 w-4" />Save search</Button><Button variant="outline" onClick={load} disabled={loading}><RefreshCw className={`mr-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} />Refresh</Button></div>
        </div>
      ...
      {loading ? <div className="titan-surface p-6 text-sm text-muted-foreground">Loading opportunities…</div> : visible.length === 0 ? <EmptyState icon={Search} title="No jobs match these filters" description="Broaden the filters or refresh the opportunity feed." actionLabel="Clear filters" onAction={() => setFilters(DEFAULT_FILTERS)} /> : <section className="grid gap-4 lg:grid-cols-2">{visible.map((job) => {
        ...
        return <article key={`${job.source || "titan"}-${job.id}`} className="titan-surface p-5 space-y-4">
          ...
          <div className="grid gap-2 sm:grid-cols-2"><Button variant="outline" disabled={busyId === job.id} onClick={() => track(job, "saved")}>Save</Button><Button variant="outline" disabled={busyId === job.id} onClick={() => track(job, "applied")}>Track application</Button><Button asChild><Link to={buildResumeLink(job)}><FileText className="mr-2 h-4 w-4" />Tailor resume</Link></Button>{isExternalJobMatch(job) && job.source_url ? <Button asChild variant="outline"><a href={job.source_url} target="_blank" rel="noopener noreferrer">Original listing<ExternalLink className="ml-2 h-4 w-4" /></a></Button> : <Button asChild variant="outline"><Link to={`/hire?tab=browse&job=${encodeURIComponent(job.id)}`}>Open Titan job</Link></Button>}</div>
        </article>;
      })}</section>}
