...
begin
  ...
  values (p_bucket_key, 0, v_now, v_now) on conflict (bucket_key) do nothing;
  select * into v_row from public.titan_rate_limit_buckets where bucket_key = p_bucket_key for update;
  if v_row.window_started_at + make_interval(secs => v_window_seconds) <= v_now then
    update public.titan_rate_limit_buckets set request_count = 1, window_started_at = v_now, updated_at = v_now where bucket_key = p_bucket_key;
    return query select true, 0; return;
  end if;
  if v_row.request_count >= v_limit then
    return query select false, greatest(1, ceil(extract(epoch from ((v_row.window_started_at + make_interval(secs => v_window_seconds)) - v_now)))::integer); return;
  end if;
  update public.titan_rate_limit_buckets set request_count = request_count + 1, updated_at = v_now where bucket_key = p_bucket_key;
...
comment on function public.consume_rate_limit(text, integer, integer) is 'Server-only durable fixed-window rate limiter used by sensitive TitanOS API routes.';
