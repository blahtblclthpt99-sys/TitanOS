...
export default function RoutePlanner() {
  ...
  return <div className="p-4 md:p-8 max-w-6xl mx-auto pb-24">
    ...
    <div className="glass rounded-2xl p-5 border border-titan-cyan/15 mb-6 flex flex-col sm:flex-row gap-4 sm:items-center justify-between">
      <div><p className="text-sm text-white/50">Estimated route distance</p><p className="text-3xl font-bold text-white">{route.totalMiles.toLocaleString()} <span className="text-base text-white/45">miles</span></p><p className="text-xs text-white/35 mt-1">Nearest-neighbor estimate based on saved locations.</p></div>
    ...
    {loading ? <p className="text-white/45">Loading route…</p> : !jobs.length ? <div className="glass rounded-2xl p-12 text-center border border-white/8"><Route className="w-9 h-9 text-titan-cyan mx-auto mb-3" /><p className="font-semibold text-white">No scheduled stops today</p><p className="text-sm text-white/40 mt-1">Schedule jobs to build a route.</p></div> : <div className="grid lg:grid-cols-2 gap-6">
      <section className="glass rounded-2xl p-5 border border-white/8"><h2 className="font-semibold text-white mb-4">Original order</h2><div className="space-y-3">{jobs.map((job, index) => <Stop key={job.id} index={index + 1} job={job} />)}</div></section>
...
function Stop({ index, job }) {
  return <div className="rounded-xl bg-white/[0.03] p-3 flex gap-3"><span className="w-7 h-7 rounded-full bg-titan-cyan/15 text-titan-cyan text-xs font-bold flex items-center justify-center flex-none">{index}</span><div className="min-w-0"><p className="text-sm font-medium text-white truncate">{label(job)}</p><p className="text-xs text-white/40 mt-1 flex items-center gap-1"><MapPin className="w-3 h-3" />{location(job)}</p></div></div>;
}
