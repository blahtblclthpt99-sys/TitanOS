...
export default function Notifications() {
  ...
  return <div className="relative p-4 md:p-8 max-w-3xl mx-auto pb-32">
    ...
    <div className="relative"><div className="flex items-start justify-between gap-4"><PageHeader title="Notifications" subtitle="Stay on top of your TitanOS activity" /><Button onClick={readAll} disabled={!hasUnread || saving} variant="outline" className="border-border text-foreground hover:bg-muted rounded-xl shrink-0">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <><CheckCheck className="w-4 h-4 mr-2" />Mark all read</>}</Button></div>{betaBadgeLabel() && <div className="glass rounded-2xl mb-5 px-4 py-2 border border-titan-cyan/20 text-xs font-semibold text-titan-cyan">{betaBadgeLabel()}</div>}
      {loading ? <PageLoader variant="list" label="Loading notifications" /> : notifications.length ? <div className="glass rounded-2xl border border-border overflow-hidden">{notifications.map((notification) => <button key={notification.id} onClick={() => openNotification(notification)} className={`w-full text-left p-5 flex gap-4 border-b border-border last:border-0 transition-colors hover:bg-muted/60 ${notification.read_at ? "opacity-60" : "bg-titan-cyan/[0.035]"}`}><div className={`mt-1 w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${notification.read_at ? "bg-muted text-muted-foreground" : "bg-titan-cyan/15 text-titan-cyan"}`}><Bell className="w-4 h-4" /></div><div className="flex-1 min-w-0"><div className="flex items-start justify-between gap-3"><p className="font-medium text-foreground">{notification.title}</p><span className="text-xs text-muted-foreground whitespace-nowrap">{timeAgo(notification.created_at || notification.created_date)}</span></div>{notification.body && <p className="text-sm text-foreground/45 mt-1">{notification.body}</p>}</div><ChevronRight className="w-4 h-4 text-muted-foreground self-center shrink-0" /></button>)}</div> : <div className="glass rounded-3xl p-12 text-center border border-border"><Bell className="w-9 h-9 text-titan-cyan mx-auto mb-3" /><p className="font-semibold text-foreground">You're all caught up</p><p className="text-sm text-muted-foreground mt-1">New activity and updates will appear here.</p></div>}
    </div>
  </div>;
}
