...
export default function Hire() {
  ...
  return <div className="relative p-4 md:p-8 max-w-6xl mx-auto pb-32">
    ...
    <Dialog open={!!messageTarget} onOpenChange={(open) => !open && setMessageTarget(null)}><DialogContent className="bg-titan-surface1 border-border text-foreground max-w-md rounded-2xl">{messageTarget && <><DialogHeader><DialogTitle>Message {messageTarget.recipientName}</DialogTitle></DialogHeader><div className="max-h-64 overflow-y-auto space-y-3 pr-1">{messages.filter((message) => message.sender_id === user.id || message.sender_id === messageTarget.recipientId).map((message) => <div key={message.id} className={`rounded-xl p-3 text-sm ${message.sender_id === user.id ? "bg-titan-cyan/15 ml-8" : "bg-muted mr-8"}`}><p className="text-foreground/80">{message.body}</p></div>)}</div><form onSubmit={submitMessage} className="flex gap-2 pt-2"><Textarea value={messageBody} onChange={(event) => setMessageBody(event.target.value)} placeholder="Write a message…" className={fieldClass} rows={2} /><Button type="submit" disabled={saving || !messageBody.trim()} className="bg-titan-cyan text-black self-end"><Send className="w-4 h-4" /></Button></form></>}</DialogContent></Dialog>
  </div>;
}
