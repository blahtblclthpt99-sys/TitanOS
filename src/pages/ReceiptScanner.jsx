...
export default function ReceiptScanner() {
  ...
  return <div className="p-4 md:p-8 max-w-3xl mx-auto pb-24">
    ...
    <div className="glass rounded-2xl p-6 border border-border space-y-5">
      ...
      <Button type="button" onClick={parse} disabled={parsing || (!file && !pastedText.trim())} className="w-full font-semibold">{parsing ? <Loader2 className="w-4 h-4 animate-spin" /> : <><ScanLine className="w-4 h-4 mr-2" />Parse receipt</>}</Button>
    </div>
    <div className="glass rounded-2xl p-6 mt-6 border border-border space-y-4">
      ...
      <Button onClick={save} disabled={saving || !receipt.vendor || !Number(receipt.amount)} className="w-full font-semibold">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : "Save deductible expense"}</Button>
    </div>
  </div>;
}
