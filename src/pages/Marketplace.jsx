...
function ListingCard({ listing, favorite, onFavorite, onOpen, index, reduceMotion }) {
  return <motion.article {...listItemMotion(reduceMotion, index)} {...hoverLiftMotion(reduceMotion)} className="titan-surface overflow-hidden border border-border hover:border-titan-cyan/25 transition-colors">
  ...
  return <div className="relative p-4 md:p-8 max-w-7xl mx-auto min-h-full">
    ...
    <div className="relative"><PageHeader title="Marketplace" subtitle="Find trusted local services and share your expertise" onAdd={() => { setListingForm(blankListing); setFormOpen(true); }} addLabel="Post a Service" />
      ...
      {tab === "apps" ? (
        ...
        {betaBadgeLabel() && <div className="titan-surface mb-5 p-4 border border-primary/20 flex items-center justify-between gap-4"><div><p className="font-semibold text-foreground">{betaBadgeLabel()}</p><p className="text-xs text-muted-foreground mt-1">Post and connect with local service providers at no cost.</p></div><span className="text-xs font-medium text-primary">{isMarketplaceFree(user) ? "No fees" : MARKETPLACE_PREMIUM.enabled ? "Premium available" : ""}</span></div>}
