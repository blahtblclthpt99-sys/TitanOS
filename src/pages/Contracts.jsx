...
export default function Contracts() {
  ...
  return <div className="p-4 md:p-8 max-w-5xl mx-auto pb-28"><PageHeader title="Contracts" subtitle="Create, share, and sign service agreements." />
    <form onSubmit={create} className="glass rounded-3xl p-5 md:p-7 border border-white/8 space-y-4"><h2 className="font-semibold text-white">New contract</h2><div className="grid sm:grid-cols-2 gap-4"><label className="text-sm text-white/60">Title<Input required value={form.title} onChange={(e) => setForm((x) => ({ ...x, title: e.target.value }))} className={`mt-1 ${inputClass}`} /></label><label className="text-sm text-white/60">Customer name<Input required value={form.customer_name} onChange={(e) => setForm((x) => ({ ...x, customer_name: e.target.value }))} className={`mt-1 ${inputClass}`} /></label></div><label className="block text-sm text-white/60">Contract body (optional)<Textarea value={form.body} onChange={(e) => setForm((x) => ({ ...x, body: e.target.value }))} rows={5} className={`mt-1 ${inputClass}`} /></label><Button disabled={saving} type="submit" className="bg-titan-cyan text-black">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : "Create contract"}</Button></form>
    <section className="mt-6 space-y-3"><h2 className="font-semibold text-white">Your contracts</h2>{contracts.length ? contracts.map((contract) => <article key={contract.id} className="glass rounded-2xl p-5 border border-white/8 flex flex-col sm:flex-row gap-4 sm:items-center"><div className="flex-1"><h3 className="font-semibold text-white">{contract.title}</h3><p className="text-sm text-white/45 mt-1">{contract.customer_name || "Customer not named"}</p><span className={`inline-block mt-2 px-2 py-1 rounded-md text-xs capitalize ${contract.status === "signed" ? "bg-emerald-400/15 text-emerald-300" : contract.status === "sent" ? "bg-titan-cyan/10 text-titan-cyan" : "bg-white/5 text-white/50"}`}>{contract.status || "draft"}</span></div><div className="flex gap-2"><Button onClick={() => copy(contract)} variant="outline" className="border-white/15 text-white"><Copy className="w-4 h-4 mr-1" />Copy link</Button>{!contract.owner_signature && <Button onClick={() => sign(contract)} disabled={saving} className="bg-titan-cyan text-black"><PenLine className="w-4 h-4 mr-1" />Sign</Button>}</div></article>) : <p className="glass rounded-2xl p-6 text-sm text-white/40">No contracts yet.</p>}</section>
  </div>;
}
