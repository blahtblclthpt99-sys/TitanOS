...
function skillScore(profile, job) {
  ...
  if (!inferred.length || !profile.skills.length) {
...
function locationScore(profile, job) {
  ...
  if (job.work_mode === "remote") return { ratio: 1, reason: "Remote" };
...
function dedupeKey(job = {}) {
  ...
  if (direct) return `url:${direct.replace(/\?.*$/, "")}`;
  return [clean(job.title), clean(job.city), clean(job.state), clean(job.source_name)].join("|");
}
...
export function mergeRankedJobMatches({ internal = [], external = [], driverProfile = {}, now = Date.now() } = {}) {
  ...
  const seen = new Set(rankedInternal.map(dedupeKey));
