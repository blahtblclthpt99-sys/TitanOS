CREATE TABLE IF NOT EXISTS public.insurance_docs (
  ...
  user_id TEXT NOT NULL, name TEXT NOT NULL DEFAULT '', url TEXT NOT NULL DEFAULT '', doc_type TEXT NOT NULL DEFAULT 'liability', carrier TEXT DEFAULT '', policy_number TEXT DEFAULT '', expires_on DATE, size_label TEXT DEFAULT '', notes TEXT DEFAULT ''
);
CREATE TABLE IF NOT EXISTS public.portal_actions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now(), customer_id TEXT NOT NULL, action TEXT NOT NULL, entity_type TEXT DEFAULT '', entity_id TEXT DEFAULT '', meta JSONB DEFAULT '{}'::jsonb
);
...
DO $$ BEGIN CREATE POLICY insurance_docs_own ON public.insurance_docs FOR ALL TO authenticated USING (user_id=auth.uid()::text) WITH CHECK (user_id=auth.uid()::text); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY portal_actions_service ON public.portal_actions FOR ALL TO service_role USING (true) WITH CHECK (true); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
