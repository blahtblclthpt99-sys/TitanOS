...
alter policy activity_read on public.activity_events
  using (created_by_id = (select auth.uid()) or (select public.is_admin()));
...
alter policy driver_profiles_select on public.driver_profiles
  using (published = true or user_id = (select auth.uid()) or (select public.is_admin()));
alter policy driver_profiles_insert on public.driver_profiles
  with check (user_id = (select auth.uid()) and created_by_id = (select auth.uid()));
...
alter policy employees_insert on public.employees
  with check (created_by_id = (select auth.uid()) or (select public.is_admin()));
...
alter policy listings_select on public.marketplace_listings
  using (status = 'active' or created_by_id = (select auth.uid()) or (select public.is_admin()));
alter policy listings_write on public.marketplace_listings
  using (created_by_id = (select auth.uid()) or (select public.is_admin()))
  with check ((select public.is_admin()) or (created_by_id = (select auth.uid()) and seller_id = (select auth.uid())::text));

alter policy messages_select on public.marketplace_messages
  using (sender_id = (select auth.uid())::text or recipient_id = (select auth.uid())::text or created_by_id = (select auth.uid()) or (select public.is_admin()));
alter policy messages_insert on public.marketplace_messages
  with check (sender_id = (select auth.uid())::text and created_by_id = (select auth.uid()));
alter policy messages_update on public.marketplace_messages
  using (sender_id = (select auth.uid())::text or recipient_id = (select auth.uid())::text or (select public.is_admin()))
  with check ((select public.is_admin()) or sender_id = (select auth.uid())::text or recipient_id = (select auth.uid())::text);
alter policy messages_delete on public.marketplace_messages
  using (sender_id = (select auth.uid())::text or created_by_id = (select auth.uid()) or (select public.is_admin()));

alter policy notifications_select on public.notifications
  using (user_id = (select auth.uid())::text or created_by_id = (select auth.uid()) or (select public.is_admin()));
alter policy notifications_insert on public.notifications
  with check (user_id = (select auth.uid())::text and created_by_id = (select auth.uid()));
