...
import { cleanSupportMessage, isSupportAdmin, loadAssignedSupportCase, writeSupportAudit } from "../_lib/support.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "create") {
      ...
      if (error) throw error;
      await writeSupportAudit(auth.admin, { actorUserId: auth.user.id, action: "support_incident_created", targetType: "support_incident", targetId: data.id, metadata: { severity } });
    ...
    if (action === "update") {
      ...
      await writeSupportAudit(auth.admin, { actorUserId: auth.user.id, action: "support_incident_updated", targetType: "support_incident", targetId: incidentId, metadata: { status } });
    ...
    if (action === "link_case") {
      ...
      if (error) throw error;
      await auth.admin.from("support_case_events").insert({ case_id: caseId, actor_user_id: auth.user.id, event_type: "incident_linked", details: { incident_id: incidentId, incident_title: incident.title } });
      await writeSupportAudit(auth.admin, { caseId, actorUserId: auth.user.id, action: "support_incident_linked", targetType: "support_incident", targetId: incidentId });
