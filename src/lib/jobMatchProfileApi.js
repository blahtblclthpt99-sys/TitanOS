...
export async function getMyJobMatchPreferences(userId) {
  ...
  const { data, error } = await supabase
    ...
    .select("user_id,skills,certifications,years_experience,city,state,availability,job_interests,work_radius_miles,desired_pay_min,desired_pay_type,preferred_schedule,external_job_search_consent,external_job_search_consent_at")
    .eq("user_id", userId)
    .maybeSingle();
