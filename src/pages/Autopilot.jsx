    ...
    <div className="page-pad max-w-5xl mx-auto pb-24">
      ...
      {checkout === "success" && returnedOrder && (
        <section className="titan-surface p-5 mb-5 border border-emerald-500/30">
          <div className="flex items-start gap-3"><CheckCircle2 className="w-6 h-6 text-emerald-400" /><div className="flex-1"><h2 className="font-semibold">Sprint approved</h2><p className="text-sm text-muted-foreground mt-1">Run the approved sprint once. Duplicate sends are prevented.</p></div></div>
          <Button className="mt-4 min-h-11" onClick={runOrder} disabled={working}>{working ? "Checking approval…" : "Send approved reminders"}</Button>
        </section>
      )}
      <section className="titan-surface p-5 mb-5">
        <div className="flex gap-3 items-start"><Bot className="w-7 h-7 text-titan-cyan" /><div><h2 className="text-lg font-semibold">Invoice Recovery Sprint</h2><p className="text-sm text-muted-foreground mt-1">One approved email reminder for up to 10 overdue invoices. Failed delivery is logged and duplicate runs are prevented.</p></div></div>
      ...
      <section className="titan-surface p-5">
        ...
        {eligible.length > 0 && <Button className="w-full sm:w-auto mt-5 min-h-11" disabled={!selected.length || working} onClick={checkoutNow}>{working ? "Preparing sprint…" : <>Continue with approved recipients <ExternalLink className="w-4 h-4" /></>}</Button>}
      </section>
    </div>
  );
}
