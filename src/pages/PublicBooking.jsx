...
export default function PublicBooking() {
  ...
  if (status === "loading") return <div className="min-h-screen bg-[#0d0e12] grid place-items-center text-muted-foreground"><Loader2 className="animate-spin w-6 h-6" /></div>;
  if (status === "missing") return <main className="min-h-screen bg-[#0d0e12] grid place-items-center p-6 text-center"><div className="glass rounded-3xl p-8 text-foreground"><h1 className="text-xl font-semibold">Booking page unavailable</h1><p className="text-foreground/45 mt-2">This page may be unpublished or no longer available.</p></div></main>;
  if (status === "success") return <main className="min-h-screen bg-[#0d0e12] grid place-items-center p-6 text-center"><div className="glass rounded-3xl p-8 max-w-md"><h1 className="text-2xl font-bold text-foreground">Request sent</h1><p className="text-muted-foreground mt-3">Thanks — {page.title} will get back to you shortly.</p></div></main>;

  const openDays = availability.filter((slot) => slot.is_open !== false);
  return <main className="min-h-screen bg-[#0d0e12] text-foreground p-4 md:p-8"><div className="max-w-3xl mx-auto"><header className="glass rounded-3xl p-6 md:p-8 border border-border"><h1 className="text-3xl font-bold">{page.title}</h1>{page.verified_badge && <p className="mt-3 text-sm text-titan-cyan flex items-center gap-1.5"><ShieldCheck className="w-4 h-4" />Verified provider</p>}<p className="text-muted-foreground mt-4 whitespace-pre-line">{page.bio}</p><div className="mt-5 flex flex-wrap gap-2">{(page.services || []).map((service) => <span key={service} className="rounded-full bg-titan-cyan/10 text-titan-cyan px-3 py-1 text-sm">{service}</span>)}</div><p className="text-sm text-foreground/45 mt-5">{[page.city, page.state].filter(Boolean).join(", ") || page.service_area || "Service area available on request"}</p></header>
