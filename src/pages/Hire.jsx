...
function JobCard({ job, saved, onSave, onApply }) {
  return <article className="glass rounded-2xl p-5 border border-white/8">
    <div className="flex justify-between gap-3">
      <div className="min-w-0"><h2 className="font-semibold text-white truncate">{job.title}</h2><p className="text-xs text-titan-cyan mt-1">{job.category}</p>{(job.is_same_day || job.is_urgent) && <div className="flex gap-1 mt-2">{job.is_same_day && <span className="text-[10px] px-2 py-0.5 rounded-full bg-titan-amber/15 text-titan-amber">Same day</span>}{job.is_urgent && <span className="text-[10px] px-2 py-0.5 rounded-full bg-red-400/15 text-red-300">Urgent</span>}</div>}</div>
      <div className="flex items-start gap-2"><button type="button" onClick={() => onSave(job.id)} aria-label={saved ? "Remove saved job" : "Save job"} className="p-1 text-white/45 hover:text-titan-cyan"><Bookmark className={`w-5 h-5 ${saved ? "fill-titan-cyan text-titan-cyan" : ""}`} /></button><p className="text-xs text-white/35 whitespace-nowrap">{timeAgo(job.created_date || job.created_at)}</p></div>
    </div>
    <p className="text-sm text-white/55 leading-relaxed mt-3 line-clamp-2">{job.description}</p>
    <div className="flex justify-between items-end mt-4 pt-3 border-t border-white/6">
      <div className="space-y-1 text-xs text-white/40"><p className="flex gap-1 items-center"><MapPin className="w-3 h-3" />{locationLabel(job.city, job.state) || "Location not provided"}</p>{job.deadline && <p className="flex gap-1 items-center"><CalendarDays className="w-3 h-3" />Due {new Date(`${job.deadline}T12:00:00`).toLocaleDateString()}</p>}</div>
...
export default function Hire() {
  ...
  return <div className="relative p-4 md:p-8 max-w-6xl mx-auto pb-32">
    ...
    <div className="relative">
      ...
      <div className="flex overflow-x-auto gap-2 mb-6 border-b border-white/8">{tabs.map(([id, label]) => <button key={id} onClick={() => setTab(id)} className={`px-4 py-3 text-sm font-medium whitespace-nowrap border-b-2 transition-colors ${tab === id ? "border-titan-cyan text-titan-cyan" : "border-transparent text-white/40 hover:text-white/70"}`}>{label}</button>)}</div>
      {tab === "browse" && <><div className="glass rounded-2xl p-4 mb-6 border border-white/8 grid grid-cols-1 sm:grid-cols-3 gap-3"><div className="relative"><Search className="absolute left-3 top-3 w-4 h-4 text-white/30" /><Input value={filters.search} onChange={(e) => updateFilter("search", e.target.value)} placeholder="Search jobs" className={`${fieldClass} pl-9`} /></div><select value={filters.category} onChange={(e) => updateFilter("category", e.target.value)} className={fieldClass}><option value="All">All categories</option>{SERVICE_CATEGORIES.map((value) => <option key={value}>{value}</option>)}</select><select value={filters.state} onChange={(e) => updateFilter("state", e.target.value)} className={fieldClass}><option value="">All states</option>{US_STATES.map((value) => <option key={value}>{value}</option>)}</select></div>{loading ? <PageLoader variant="list" label="Loading jobs" /> : jobs.length ? <div className="grid md:grid-cols-2 gap-4">{jobs.map((job) => <JobCard key={job.id} job={job} saved={savedIds.has(job.id)} onSave={saveToggle} onApply={openApply} />)}</div> : <EmptyPanel title="No jobs found" detail="Try broadening your search or check back soon." />}</>}
    ...
    <Dialog open={!!messageTarget} onOpenChange={(open) => !open && setMessageTarget(null)}><DialogContent className="bg-titan-surface1 border-white/10 text-white max-w-md rounded-2xl">{messageTarget && <><DialogHeader><DialogTitle>Message {messageTarget.recipientName}</DialogTitle></DialogHeader><div className="max-h-64 overflow-y-auto space-y-3 pr-1">{messages.filter((message) => message.sender_id === user.id || message.sender_id === messageTarget.recipientId).map((message) => <div key={message.id} className={`rounded-xl p-3 text-sm ${message.sender_id === user.id ? "bg-titan-cyan/15 ml-8" : "bg-white/5 mr-8"}`}><p className="text-white/80">{message.body}</p></div>)}</div><form onSubmit={submitMessage} className="flex gap-2 pt-2"><Textarea value={messageBody} onChange={(event) => setMessageBody(event.target.value)} placeholder="Write a message…" className={fieldClass} rows={2} /><Button type="submit" disabled={saving || !messageBody.trim()} className="bg-titan-cyan text-black self-end"><Send className="w-4 h-4" /></Button></form></>}</DialogContent></Dialog>
  </div>;
}
