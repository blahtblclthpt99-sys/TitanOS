...
export default function Leads() {
  ...
  return <div className="p-4 md:p-8 max-w-6xl mx-auto"><PageHeader title="Leads" subtitle="Find, qualify, and convert new work" /><form onSubmit={add} className="glass rounded-2xl p-4 flex gap-2 mb-4"><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Lead name" className="bg-muted border-border text-foreground" /><Button className="bg-titan-cyan text-black"><Plus className="w-4 h-4" />Add lead</Button></form><div className="flex gap-2 overflow-auto mb-4">{["all", ...STATUSES].map((status) => <Button key={status} onClick={() => setFilter(status)} variant="outline" className={filter === status ? "border-titan-cyan text-titan-cyan" : "border-border text-muted-foreground"}>{status}</Button>)}</div><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">{display.map((row) => <article key={row.id} className="glass rounded-2xl p-4"><UserRound className="w-5 h-5 text-titan-cyan mb-3" /><p className="font-semibold text-foreground">{row.name}</p><p className="text-xs text-muted-foreground my-2">{row.phone || row.email || "New inquiry"}</p><select value={row.status} onChange={(e) => move(row, e.target.value)} className="w-full bg-muted border border-border rounded-lg p-2 text-sm text-foreground">{STATUSES.map((s) => <option key={s}>{s}</option>)}</select></article>)}</div></div>;
}
