...
import { contractsExportSpec } from "@/lib/export";
import PageLoader from "@/components/shared/PageLoader";
import EmptyState from "@/components/shared/EmptyState";
import { useAuth } from "@/lib/AuthContext";
import { contractPublicUrl, createContract, createContractShareToken, deleteContract, listContracts, signContract } from "@/lib/contractsApi";
...
export default function Contracts() {
  ...
  const copy = async (contract) => {
    if (sharingId) return;
    setSharingId(contract.id);
    try {
      const token = await createContractShareToken(contract.id);
      await navigator.clipboard.writeText(contractPublicUrl(token));
      toast({ title: "Signing link copied", description: "Creating a new link invalidates the previous signing link." });
    } catch {
      toast({ variant: "destructive", title: "Couldn't create signing link" });
    } finally {
      setSharingId(null);
    }
  };
  ...
  return <div className="p-4 md:p-8 max-w-5xl mx-auto pb-28"><PageHeader title="Contracts" subtitle="Create, share, and sign service agreements." actions={<ExportMenu spec={contractsExportSpec(contracts)} size="sm" />} />
    <form onSubmit={create} className="titan-surface p-5 md:p-7 border border-border space-y-4"><h2 className="font-semibold text-foreground">New contract</h2><div className="grid sm:grid-cols-2 gap-4"><label className="text-sm text-muted-foreground">Title<Input required value={form.title} onChange={(e) => setForm((x) => ({ ...x, title: e.target.value }))} className={`mt-1 ${inputClass}`} /></label><label className="text-sm text-muted-foreground">Customer name<Input required value={form.customer_name} onChange={(e) => setForm((x) => ({ ...x, customer_name: e.target.value }))} className={`mt-1 ${inputClass}`} /></label></div><label className="block text-sm text-muted-foreground">Contract body (optional)<Textarea value={form.body} onChange={(e) => setForm((x) => ({ ...x, body: e.target.value }))} rows={5} className={`mt-1 ${inputClass}`} /></label><Button disabled={saving} type="submit">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : "Create contract"}</Button></form>
    <section className="mt-6 space-y-3"><h2 className="font-semibold text-foreground">Your contracts</h2>{contracts.length ? contracts.map((contract) => <article key={contract.id} className="titan-surface p-5 border border-border flex flex-col sm:flex-row gap-4 sm:items-center"><div className="flex-1"><h3 className="font-semibold text-foreground">{contract.title}</h3><p className="text-sm text-foreground/45 mt-1">{contract.customer_name || "Customer not named"}</p><span className={`inline-block mt-2 px-2 py-1 rounded-md text-xs capitalize ${contract.status === "signed" ? "bg-emerald-400/15 text-emerald-300" : contract.status === "sent" ? "bg-titan-cyan/10 text-titan-cyan" : "bg-muted text-muted-foreground"}`}>{contract.status || "draft"}</span></div><div className="flex gap-2 items-center"><Button onClick={() => copy(contract)} disabled={Boolean(sharingId)} variant="outline" className="border-border text-foreground">{sharingId === contract.id ? <Loader2 className="w-4 h-4 mr-1 animate-spin" /> : <Copy className="w-4 h-4 mr-1" />}Copy link</Button>{!contract.owner_signature && <Button onClick={() => sign(contract)} disabled={saving} className="bg-primary text-primary-foreground"><PenLine className="w-4 h-4 mr-1" />Sign</Button>}<DeleteButton label={contract.title || "this contract"} onDelete={async () => { await deleteContract(user.id, contract.id); setContracts((items) => items.filter((item) => item.id !== contract.id)); }} /></div></article>) : <EmptyState title="No contracts yet" description="Create a service agreement above to share a signing link." />}</section>
  </div>;
}
