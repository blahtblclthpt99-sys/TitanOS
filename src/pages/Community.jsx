...
export default function Community() {
  ...
  return <div className="relative p-4 md:p-8 max-w-6xl mx-auto pb-32">
    ...
    <div className="relative"><PageHeader title="Community" subtitle="Share professional wins with your local service network" />
      ...
      <div className="glass rounded-2xl p-4 border border-titan-cyan/15 flex gap-3 mb-6"><ShieldCheck className="w-5 h-5 text-titan-cyan shrink-0 mt-0.5" /><div className="text-sm text-white/55 space-y-1"><p>Privacy first: posts never show customer names, companies, addresses, or dollar amounts.</p><p>Keep it professional — foul language is blocked. Limit: {COMMUNITY_POST_LIMIT} posts per 12 hours.</p></div></div>
      <div className="grid lg:grid-cols-[minmax(0,1fr)_300px] gap-6">
        <div className="space-y-5">
          {canCompose ? <form onSubmit={submitPost} className="glass rounded-2xl p-5 border border-white/8"><div className="flex items-center justify-between gap-2 mb-3"><div className="flex items-center gap-2"><Sparkles className="w-4 h-4 text-titan-cyan" /><p className="font-semibold text-white">Share an update</p></div><p className="text-xs text-white/40">{quota.remaining}/{COMMUNITY_POST_LIMIT} left (12h)</p></div><Textarea required maxLength={500} rows={3} value={compose.description} onChange={(e) => setCompose((current) => ({ ...current, description: e.target.value }))} placeholder="Share a project win or useful tip. Keep customer details private." className={fieldClass} /><div className="grid sm:grid-cols-2 gap-3 mt-3"><select value={compose.category} onChange={(e) => setCompose((current) => ({ ...current, category: e.target.value }))} className={fieldClass}>{SERVICE_CATEGORIES.map((value) => <option key={value}>{value}</option>)}</select><div className="relative"><ImagePlus className="absolute left-3 top-3 w-4 h-4 text-white/30" /><Input type="url" value={compose.photoUrl} onChange={(e) => setCompose((current) => ({ ...current, photoUrl: e.target.value }))} placeholder="Photo URL (optional)" className={`${fieldClass} pl-9`} /></div></div><Button disabled={saving || quota.remaining <= 0} type="submit" className="mt-3 bg-titan-cyan text-black">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : quota.remaining <= 0 ? "Limit reached (12h)" : <><Send className="w-4 h-4 mr-2" />Share Post</>}</Button></form> : <div className="glass rounded-2xl p-5 border border-white/8"><p className="font-semibold text-white">Join the conversation</p><p className="text-sm text-white/45 mt-1">Enable Community sharing in your privacy settings to share updates.</p><Link to="/settings" className="inline-block mt-3 text-sm font-medium text-titan-cyan hover:text-white">Open Privacy Settings</Link></div>}
