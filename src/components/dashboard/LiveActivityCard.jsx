    ...
    <div className="glass rounded-2xl overflow-hidden mb-5">
      <div className="flex items-center justify-between px-5 pt-5 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-white/70 uppercase tracking-widest">Live Activity</h2>
        </div>
        <Link to="/community" className="text-xs text-white/30 hover:text-titan-cyan flex items-center gap-1">
          Community <ChevronRight className="w-3 h-3" />
        </Link>
      </div>
      <div className="px-5 pb-5 space-y-2">
        {events.length === 0 ? (
          <p className="text-sm text-white/25 py-2">No live activity yet — complete a job or share in Community.</p>
        ) : (
          events.slice(0, 5).map((event) => (
            <div key={event.id} className="flex items-start gap-3 py-2 border-b border-white/5 last:border-0">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm text-white/80 leading-snug">{event.summary}</p>
