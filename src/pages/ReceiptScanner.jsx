...
export default function ReceiptScanner() {
  ...
  const parse = async () => {
    ...
    try {
      ...
      const parsed = parseReceiptText(raw || pastedText);
      setReceipt({
        vendor: parsed.vendor || "",
        amount: parsed.amount != null && parsed.amount !== "" ? String(parsed.amount) : "",
        date: parsed.date || todayISO(),
        category: parsed.category || "other",
        raw_text: parsed.raw_text || raw || pastedText || "",
      });
      if (!raw && file) {
        toast({
          title: "Couldn't read image",
          description: "Enter vendor and amount manually — OCR found no text.",
        });
      } else {
        toast({
      ...
      toast({ variant: "destructive", title: "Couldn't parse receipt" });
    } finally {
      setParsing(false);
    }
  };
  ...
  const save = async () => {
    ...
    try {
      let receipt_url = "";
      if (file) {
        ({ file_url: receipt_url } = await api.integrations.Core.UploadFile({ file }));
      }
      const expense = await api.entities.Expense.create({
        description: receipt.vendor.trim(),
      ...
      try {
        await api.entities.ReceiptScan.create({
          expense_id: expense.id,
          receipt_url,
          raw_text: receipt.raw_text || pastedText,
          vendor: receipt.vendor.trim(),
          amount: Number(receipt.amount),
          date: receipt.date,
          category: receipt.category,
        ...
        toast({
          title: "Expense saved",
          description: "Receipt log skipped — expense is still in Finances.",
        });
        setFile(null);
        setPreview("");
        setPastedText("");
    ...
    <div className="p-4 md:p-8 max-w-3xl mx-auto pb-24">
      ...
      <div className="titan-surface p-6 border border-border space-y-5">
        ...
        {preview ? (
          ...
          <button
            ...
            className="w-full h-36 rounded-xl border border-dashed border-border hover:border-titan-cyan/50 flex flex-col items-center justify-center gap-2"
          >
            <Camera className="w-7 h-7 text-titan-cyan" />
            <span className="text-sm text-muted-foreground">Upload receipt image</span>
          </button>
        )}
        {preview && (
          <Button type="button" variant="outline" onClick={() => inputRef.current?.click()} className="border-border text-foreground">
        ...
        <Textarea
          ...
          rows={4}
          placeholder="Optional: paste OCR text from the receipt"
      ...
      <div className="titan-surface p-6 mt-6 border border-border space-y-4">
        ...
        <NativeSelect
          ...
          onValueChange={(category) => change("category", category)}
          placeholder="Category"
          options={EXPENSE_CATEGORIES.map((item) => ({ value: item.id, label: item.label }))}
        />
        <Button
          onClick={save}
          disabled={saving || !receipt.vendor || !Number(receipt.amount)}
          className="w-full font-semibold"
        >
          {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : "Save deductible expense"}
        </Button>
        <div className="flex gap-2">
          <Button type="button" variant="outline" className="flex-1 border-border" onClick={() => navigate("/finances")}>
            Finances
          </Button>
          <Button type="button" variant="outline" className="flex-1 border-border" onClick={() => navigate("/tax-center")}>
            Tax Center
          </Button>
        </div>
      </div>
